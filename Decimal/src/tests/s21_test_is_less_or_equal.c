...
void run_less_or_equal_test(s21_decimal decimal_a, s21_decimal decimal_b,
                            int correct_answer) {
  int tested_value = s21_is_less_or_equal(decimal_a, decimal_b);
...
START_TEST(test_suite_1_simple_level) {
  ...
  make_zero(&decimal_b);
  decimal_b.bits[0] = 1;
  run_less_or_equal_test(decimal_a, decimal_b, 1);  // (a < b)
  set_bit(127, 1, &decimal_b);
  run_less_or_equal_test(decimal_a, decimal_b, 0);  // (a > b)
...
START_TEST(test_with_zero) {
  ...
  run_less_or_equal_test(decimal_a, decimal_b, 1);  // (a == b) == 0
  set_bit(127, 1, &decimal_a);
  run_less_or_equal_test(decimal_a, decimal_b, 1);  // (a == b) == 0
  set_bit(127, 1, &decimal_b);
  run_less_or_equal_test(decimal_a, decimal_b, 1);  // (a == b) == 0
...
Suite *suite_s21_is_less_or_equal(void) {
  Suite *s = suite_create("\033[40m\033[94ms21_is_less_or_equal\033[0m");
