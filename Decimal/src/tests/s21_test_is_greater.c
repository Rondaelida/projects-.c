...
void run_greater_test(s21_decimal decimal_a, s21_decimal decimal_b,
                      int correct_answer) {
  int tested_value = s21_is_greater(decimal_a, decimal_b);
...
START_TEST(test_suite_1_simple_level) {
  ...
  make_zero(&decimal_b);
  decimal_a.bits[0] = 1;
  run_greater_test(decimal_a, decimal_b, 1);  // (a > b)
  set_bit(127, 1, &decimal_a);
  run_greater_test(decimal_a, decimal_b, 0);  // (a < b)
...
START_TEST(test_with_zero) {
  ...
  run_greater_test(decimal_a, decimal_b, 0);  // (a == b) == 0
  set_bit(127, 1, &decimal_a);
  run_greater_test(decimal_a, decimal_b, 0);  // (a == b) == 0
  set_bit(127, 1, &decimal_b);
  run_greater_test(decimal_a, decimal_b, 0);  // (a == b) == 0
...
Suite *suite_s21_is_greater(void) {
  Suite *s = suite_create("\033[40m\033[94ms21_is_greater\033[0m");
